document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.emit("mpPageLoaded")

    //Load all comments as soon as the page loads
    socket.on("mpLoadComments", data => {
        //Load all comments
        ...
        for (var item in comments) {
            ...
            document.querySelector("#mpComment").append(li);
        }
    });
    ...
    commentButton.addEventListener("click", function() {
        ...
        socket.emit("mpComment", {"comment": comment});
        console.log("Emit to server successful!");
    });

    socket.on("mpCommentAll", data => {
