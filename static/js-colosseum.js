document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.emit("rmPageLoaded")

    //Load all comments as soon as the page loads
    socket.on("rmLoadComments", data => {
        //Load all comments
        ...
        for (var item in comments) {
            ...
            document.querySelector("#rmComment").append(li);
        }
    });
    ...
    commentButton.addEventListener("click", function() {
        ...
        socket.emit("rmComment", {"comment": comment});
        console.log("Emit to server successful!");
    });

    socket.on("rmCommentAll", data => {
